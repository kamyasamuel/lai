...
export default function Sidebar({ activeFeature, setActiveFeature, open, toggle }) {
  return (
    <aside className={`transition-all duration-300 bg-[#111] h-full
      ${open ? 'w-64' : 'w-16'} overflow-hidden border-r border-[#333]`}>
      <div className="flex items-center justify-between px-4 py-2 border-b border-[#333]">
        {open && <h2 className="text-lg font-semibold mr-2">Legal AI Africa</h2>}
      ...
      <nav className="p-2">
        {Object.entries(menuStructure).map(([section, items]) => (
          <div key={section} className="mb-4">
            {open && <h3 className="text-xs text-gray-400 uppercase mb-2 px-2">{section}</h3>}
            <ul>
                ...
                <li key={name}>
                  <button
                    ...
                    className={`flex items-center w-full text-left px-2 py-1 rounded
                              hover:bg-[#222] ${activeFeature === name ? 'bg-[#222]' : ''}`}>
